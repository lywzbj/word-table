...
if not "!COMMIT_COUNT!"=="" (
    ...
    if errorlevel 1 (
        echo [0x1b][31mError: not enough history[0x1b][0m
        exit /b 1
    )
) else (
    if "!MAIN_BRANCH!"=="" (
        ...
        if "!MAIN_BRANCH!"=="" (
            echo [0x1b][31mError: cannot auto-detect main branch[0x1b][0m
            exit /b 1
        )
    ...
    if "!DIFF_BASE!"=="" (
        echo [0x1b][31mError: cannot determine merge-base[0x1b][0m
        exit /b 1
    )
...
for /f "tokens=*" %%a in ('git rev-parse --short "!DIFF_BASE!"') do (
    echo Diff base: %%a>> "!CL_FILE!"
)
echo(>> "!CL_FILE!"

REM Count and emit commits section
...
for /f "tokens=1,*" %%a in (!CL_TMP!) do call :write_commit_entry %%a %%b "!CL_FILE!"
del "!CL_TMP!" >nul 2>&1
...
REM ========== Subroutine: write one commit entry to changelog ==========
:write_commit_entry
set CH=%~1
set SUBJ=%~2
set CLF=%~3
